...
(function (global) {
  ...
  var WordFreqSync = function WordFreqSync(options) {
    ...
    options.stopWordSets = options.stopWordSets || [
      "cjk",
      "english1",
      "english2",
      "spacy"
      ...
      process: function process(text) {
        ...
        if (options.languages.indexOf("english") !== -1) {
          ...
          words.forEach(function (word) {
            word = word
              ...
              .replace(/n[\'’]t\b/gi, "") // get rid of ~n't
              .replace(/[\'’](s|ll|d|ve)?\b/gi, ""); // get rid of ’ and '
            ...
            // count++ for the stem
            if (!(stem in stems)) stems[stem] = { count: 0, word: word };
            stems[stem].count++;
            ...
            if (
              word.length === stems[stem].word.length &&
        ...
        if (options.languages.indexOf("chinese") !== -1) {
          ...
          chunks.forEach(function processChunk(chunk) {
            ...
            var substrings = getAllSubStrings(
              chunk,
              options.maxiumPhraseLength
            );
  ...
  if (typeof define === "function" && define.amd) {
    ...
    if (require.main === module) {
      console.error(
        "Error: WordFreqSync is not meant to be executed directly."
      );
    }
  } else {
