...
async function getAllReviewsById(product_id) {
  var result = [];
  await axios.post(`/api/reviews/${product_id}`).then((res) => {
    if (!res.data.length) {
...
export default function WordcloudComponent(props) {
  var product_id = props.product;

  useEffect(() => {
    var wordfreq = new WordFreq();
    getAllReviewsById(product_id).then((corpus) => {
      wordfreq.process(corpus, (list) => {
        WordCloud("canvas", { list: list, wait: 10 });
      });
    });
  }, []);
